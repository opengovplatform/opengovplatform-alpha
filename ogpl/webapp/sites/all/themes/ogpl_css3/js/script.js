...
if($('#views-exposed-form-Catalogs-Search-page-1').length > 0) {
    if($(".containers h1:first").html()!="Search"){
        var _fielVal = "Search for " + $(".containers h1:first").html();
...
if($('#suggest-cp-block').length > 0) {
    var suggest_cp_html = $('#suggest-cp-block').html();
    $('#suggest-cp-block').remove();
    $('#contentPanel .containers').append(suggest_cp_html);
}
...
if($('.rotating-banner').length > 0){
    $(".rotating-banner a").each(function(){
