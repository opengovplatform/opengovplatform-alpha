...
describe "CMS Rotating Panel workflow (Creation/Approval/Publishing)" do
...
#To Login As CMS_Admin
  it "Validation check for Title of the Rotating Panel" do
        
        @browser1.goto("#{$Site_URL}node/add/rotating-panel")
        @browser1.text_field(:id, "edit-field-instructions-0-value").set($instruction)
        @browser1.text_field(:id, "edit-field-description-0-value").set($description)
        $img_path = $img_path.gsub("/", "\\")
        puts $img_path
        @browser1.file_field(:id, "edit-field-image-0-upload").set($img_path)
        sleep 2
        @browser1.button(:id,"edit-field-image-0-filefield-upload").click
        sleep 8
        @browser1.file_field(:id, "edit-field-smaller-image-0-upload").set($img_path)
        sleep 2
        @browser1.button(:id,"edit-field-smaller-image-0-filefield-upload").click
        sleep 8
   ...
   it "Check preview" do
      ...
      $ext = $ext.slice(0..18)
      $ext = $ext.gsub(" ", "_")
      $rot_panel_title = "#{$title}" + "_" + "#{$ext}"
      puts $rot_panel_title
      @browser1.text_field(:id, "edit-title").set($rot_panel_title)
      @browser1.browser.button(:value,"Preview").click
      @browser1.text.should include($rot_panel_title)
   ...
   it "Send to moderator for approval" do
      ...
      sleep 2
      @browser5.select_list(:id, "edit-type").select("Rotating Panel")
      @browser5.select_list(:id, "edit-sid").select("Create/Edit Content")
      @browser5.button(:value,"Apply").click
      sleep 10
      @browser5.text.should include($rot_panel_title)
      ...
      @browser5.button(:value,"Apply").click
      sleep 10
      @browser5.text.should_not include($rot_panel_title)
      @browser5.link(:text, "Home").click
      @browser5.select_list(:id, "edit-type").clear
      @browser5.select_list(:id, "edit-sid").clear
      sleep 2
  ...
  it "Send back to workflow by Moderator" do
      ...
      sleep 2
      @browser2.select_list(:id, "edit-type").select("Rotating Panel")
      @browser2.select_list(:id, "edit-sid").select("Moderator Review")
      @browser2.button(:value,"Apply").click
      sleep 10
      @browser2.text.should include($rot_panel_title)
      ...
      @browser2.button(:value,"Apply").click
      sleep 10
      @browser2.text.should_not include($rot_panel_title)
      @browser2.select_list(:id, "edit-type").clear
      @browser2.select_list(:id, "edit-sid").clear
      sleep 2
      @browser2.select_list(:id, "edit-type").select("Rotating Panel")
  ...
  it "Send to publisher for approval" do
      ...
      sleep 2
      @browser3.select_list(:id, "edit-type").select("Rotating Panel")
      @browser3.select_list(:id, "edit-sid").select("Publisher Review")
      @browser3.button(:value,"Apply").click
      sleep 10
      @browser3.text.should include($rot_panel_title)
  ...
  it "Send back to workflow by Publisher" do
      ...
      @browser3.button(:value,"Apply").click
      sleep 10
      @browser3.text.should_not include($rot_panel_title)
      @browser3.select_list(:id, "edit-type").clear
      @browser3.select_list(:id, "edit-sid").clear
      sleep 2
      @browser3.select_list(:id, "edit-type").select("Rotating Panel")
  ...
  it "Publish Rotating Panel" do
      ...
      sleep 10
      @browser3.text.should include($rot_panel_title)
      puts "Publish Rotating Panel completed"
  end
  ...
  it "Verify Rotating panel on front end" do
      driver = Selenium::WebDriver.for :firefox, :profile => "Selenium"
      @browser = Watir::Browser.new driver
      @browser.goto("#{$Site_URL}")
      @browser.html.should include($rot_panel_title)
      @browser.close
      puts "Verify Rotating panel on frontend completed"
  end
=begin
  it "Delete Rotating Panel" do
      @browser4.refresh
      @browser4.goto("#{$Site_URL}admin/content/node")
      sleep 3
      @browser4.select_list(:id, "edit-type").select("Rotating Panel")
      @browser4.button(:value,"Filter").click
      sleep 5
      @browser4.checkbox(:id, "edit-nodes-#{$node_val}").set
      #@browser4.checkbox(:id, "edit-nodes-1194").set
      @browser4.select_list(:id, "edit-operation").select("Delete")
      @browser4.button(:value,"Update").click
      sleep 3
      @browser4.text.should include($rot_panel_title)
      @browser4.button(:value,"Delete all").click
      @browser4.text.should include("Rotating Panel #{$rot_panel_title} has been deleted")
      puts "Delete Rotating Panel completed"

  end

  it "Verify Rotating panel not displayed on front end after deletion" do
      driver = Selenium::WebDriver.for :firefox, :profile => "Selenium"
      @browser = Watir::Browser.new driver
      @browser.goto("#{$Site_URL}")
      @browser.html.should_not include($rot_panel_title)
      @browser.close
      puts "Verify Rotating panel not displayed on frontend after deletion completed"
  end
=end
  after(:all) do
