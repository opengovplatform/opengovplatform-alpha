...
describe "CMS FAQ workflow (Creation/Approval/Publishing)" do
...
#To Login As CMS_Admin
  it "Validation check for Question of the FAQ" do
	...
	$aud_path = $aud_path.gsub("/", "\\")
        ...
        puts $vid_path
        @browser1.file_field(:id, "edit-field-audio-0-upload").set($aud_path)
        sleep 2
        @browser1.button(:id,"edit-field-audio-0-filefield-upload").click
        sleep 10
        @browser1.file_field(:id, "edit-field-video-0-upload").set($vid_path)
        sleep 2
        @browser1.button(:id,"edit-field-video-0-filefield-upload").click
        sleep 10
 ...
 #     @browser1.text.should include("Preview")
   ...
   it "Send to moderator for approval" do
      ...
      @browser5.button(:value,"Apply").click
      sleep 10
      @browser5.text.should_not include($quest_title)
      @browser5.link(:text, "Home").click
      @browser5.select_list(:id, "edit-type").clear
      @browser5.select_list(:id, "edit-sid").clear
      sleep 3
  ...
  it "Send back to workflow by Moderator" do
      ...
      @browser2.button(:value,"Apply").click
      sleep 10
      @browser2.text.should_not include($quest_title)
      @browser2.select_list(:id, "edit-type").clear
      @browser2.select_list(:id, "edit-sid").clear
      sleep 3
      @browser2.select_list(:id, "edit-type").select("FAQ")
  ...
  it "Send back to workflow by Publisher" do
      ...
      @browser3.button(:value,"Apply").click
      sleep 10
      @browser3.text.should_not include($quest_title)
      @browser3.select_list(:id, "edit-type").clear
      @browser3.select_list(:id, "edit-sid").clear
      sleep 3
      @browser3.select_list(:id, "edit-type").select("FAQ")
  ...
  it "Verify FAQ on front end" do
      driver = Selenium::WebDriver.for :firefox, :profile => "Selenium"
      @browser = Watir::Browser.new driver
      @browser.goto("#{$Site_URL}faq")
      sleep 5
      @browser.text.should include($quest_title)
      #@browser.text.should include($revised_body)
      @browser.close
...
=begin
  it "Delete FAQ" do
      ...
      sleep 3
      @browser4.select_list(:id, "edit-type").select("FAQ")
      ...
      sleep 3
      @browser4.text.should include($quest_title)
      @browser4.button(:value,"Delete all").click
      @browser4.text.should include("FAQ #{$quest_title} has been deleted")
      puts "Delete FAQ completed"

  end

  it "Verify FAQ not displayed on front end after deletion" do
      driver = Selenium::WebDriver.for :firefox, :profile => "Selenium"
      @browser = Watir::Browser.new driver
      @browser.goto("#{$Site_URL}faq")
      sleep 5
      @browser.text.should_not include($quest_title)
      #@browser.text.should_not include($revised_body)
      @browser.close
      puts "Verify FAQ not displayed on frontend after deletion completed"
  end
=end
  after(:all) do
